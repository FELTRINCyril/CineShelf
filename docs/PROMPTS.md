...
| ~~**Le rendu des cinq écrans de `V4` et `V5b` n'est pas assené.**~~ **Réglé par la cible `CineShelfScreenTests`** (HASH_HOST), qui a un `TEST_HOST` et sonde les sept écrans plus le contrôle négatif. **Deux défauts trouvés en la branchant** : `ImageRenderer` ne met pas en page un `ScrollView` — d'où `ScreenScroll` — et trois écrans de `V5b` portaient un `ScrollView` redondant à l'intérieur de celui de `RegularRootView`. Texte d'origine : | ✅ HASH_HOST |
...
| `L20` | **Annulation de l'édition en masse et de la fusion** — journal inversable | `02 §3.9`, `03 §12` | `L8` `L10` | ✅ **HASH_L20** — **ne touche pas au schéma**, contrairement à ce que sa fiche annonçait |
