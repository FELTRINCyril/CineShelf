...
| **La corbeille et la maintenance du bloc `7g` ne sont pas rendues** — « Vider maintenant », « Analyser ». **La logique est faite (`296eb3a`) ; c'est l'écran qui manque.** `TrashService` liste et restaure les six types, `MaintenanceService` compte les médias sans rattachement — et **aucune vue ne les appelle** : capacité écrite jamais lue | une reprise de `V7` |
