...
| ~~**Le bandeau de `I10` n'est posé sur aucun écran**~~ — **résolu en partie par `8dbb05e`** : `V10` le pose pour la synchronisation, le quota iCloud et l'échec, quand `SyncStatus.needsAttention` est vrai. Restent « hors ligne » — délibérément **sans** bandeau, c'est une information et non une interruption — et la fin d'import, qui appartient à `V8` | `V8` |
...
| ~~**La corbeille et la maintenance du bloc `7g` ne sont pas rendues**~~ — **résolu par `8dbb05e`** : la section « Synchronisation et données » des réglages porte les deux, plus l'écran de corbeille. « Vider maintenant » est rendu par « Analyser », qui lance la passe et chiffre ce qu'elle a fait | — |
...
| **`Reduce Transparency` n'est respecté nulle part, et il n'a aucune surface à respecter.** Vérifié le 2026-08-07 : `scrim/modal` et `scrim/crop` n'ont **aucun appelant**, et les seuls matériaux vivants (`PosterCard`) appartiennent au banc d'essai. Le brief dit pourtant qu'il « compte double dans un design qui utilise des voiles ». Écrire le modificateur sans appelant reproduirait le défaut « écrit jamais lu » : il se posera avec le premier voile réel | la première surface voilée |
| **La liste des grandes tailles n'est pas dessinée.** `docs/06` dit « les grilles basculent en liste » sans qu'aucune planche ne montre cette liste. La bascule est donc **une colonne flexible**, ce qui donne une affiche pleine largeur sur une fenêtre large — mesuré : 8 couleurs distinctes à `large`, **2** à `AX3`. Une vraie liste poserait une vignette **à côté** du texte. Livré dans sa forme minimale plutôt qu'inventé | une planche, puis une reprise |
| **L'absence de troncature à `AX5` n'est prouvée par rien.** La sonde balaie six crans et prouve que chacun **dessine** ; `ImageRenderer` rend des pixels et non des boîtes de texte, donc un « … » y est trois pixels comme un autre. Le brief exige « sans troncature » : ça reste à l'œil, sur appareil | une vérification manuelle |
| **« Déduplication par empreinte » du bloc `7g` n'est pas rendue.** `MaintenanceService` ne la fait pas : le dédoublonnage vit à l'import, dans `MediaRepository.findOrCreate`, par `checksum` — et **rien ne rattrape les doublons déjà en base**, par exemple ceux qu'une fusion CloudKit aurait créés en dédoublant un média entre deux appareils. Le libellé de la ligne dit donc « détection d'orphelins » seul, plutôt que de promettre un travail qui n'a pas lieu | une passe dédiée |
| ~~**Aucun test ne couvre l'écran de corbeille au-delà du rendu**~~ — **résolu par `a221243`** : la couture est jouée de bout en bout et relue depuis un contexte neuf. Reste non exercé le **bouton SwiftUI** lui-même, qui demande un test d'interface | un test d'interface |
| **La passe d'entretien est sautée quand CloudKit est actif**, et c'est une garde temporaire. Ses quatre étapes lisent « relation absente » comme « déchet » — or c'est le **régime normal d'une importation CloudKit en cours** : le miroir matérialise par lots, et les deux extrémités d'une relation peuvent arriver dans des transactions distinctes. La passe supprimerait des arêtes valides **et exporterait ces suppressions** à tous les appareils. La garde tombe quand `L17` fournit un état permettant d'attendre `upToDate` | `L17` puis `V10` |
| **`MaintenanceService` balaye la base entière à chaque lancement**, sans prédicat ni `fetchLimit` : six `fetch` complets pour les entités à corbeille, plus `MediaAttachment`, `Credit` et `MediaAsset`. Synchrone dans `init()`, donc avant la première vue. Sur un catalogue de plusieurs milliers de titres c'est mesurable, et **le rapport est jeté** (`try?`) — aucune trace de ce qui a été purgé, pas même un `ActivityEntry` | `V10` |
| **La purge invalide des `LegacyRecord` que leur documentation dit ne jamais purger automatiquement.** `LegacyRecord.entityID` pointe une entité native par UUID, hors graphe d'objets : purger l'entité laisse un pointeur mort, et le recours à la migration disparaît « au moment précis où l'on en aurait besoin », selon les mots de sa propre docstring | `L13` |
| **L'invariante `hasExactlyOneOwner` n'est toujours pas posée à l'attachement.** La fiche `L16` le demandait explicitement ; `MediaRepository.attach` ne la vérifie ni ne l'assène, et rien n'empêche `attachment.person = x` après coup. `L16` la **répare après coup** au lieu de fermer l'écart — une règle de lint sur les mutateurs de relation serait le bon filet, sur le patron de `no_relation_write_outside_core` | une passe de lint |
...
| 23 | `L16` — maintenance et corbeille : orphelins, purge à 30 jours | `V10`. **La seule maximale qui puisse vraiment finir le palier** : rien d'autre ne la réclame | **maximale** — ✅ **`296eb3a`**. Quatre étapes dont l'**ordre** porte l'idempotence. Sonde avant écriture : 1 défaut antérieur (`Person.credits` sans cascade, 2 crédits décapités sur 2). Revue adverse : **10 défauts, 2 critiques**, dont un que j'avais introduit — la passe **détruisait les médias sans rattachement**, que `detach` et la galerie traitent comme un état voulu. Elle les **compte** désormais |
| 24 | `V10` — synchronisation : indicateur, corbeille, espace occupé | — | légère — ✅ **`8dbb05e`**. Section « Synchronisation et données » du bloc `7g`, écran de corbeille, et **le premier appelant de `Banner` depuis `I10`**. `TrashService` vivait écrit-testé-sans-appelant. Deux maillons posés : `StorageFootprint.appLocations()` (rien ne savait quoi passer à `total(of:)`) et `MaintenanceService.survey()`, lecture seule. **La synchronisation réelle attend le prompt 21** |
| 25 | `V12` — passe d'accessibilité sur les écrans définitifs | — . **En dernier par nécessité** : elle porte sur les écrans définitifs, donc sur tous les autres | légère — 🔶 **`49aaa1a`**. La **bascule grille → liste** au-delà de `.accessibility1` (elle n'existait nulle part), VoiceOver sur les écrans récents, et une sonde qui balaie `xSmall` → `AX5`. **🔶 et pas ✅** : `Reduce Transparency` n'a aucune surface, le contraste et le clavier Mac demandent un œil, et la sonde ne peut pas prouver l'absence de troncature |
...
| `L17` | État de synchronisation : machine d'états, les 6 cas, espace occupé | `04 §5` | — | 🔶 **écrite et couverte en simulation** — `SyncStatus`, `SyncMachine`, les six messages et `StorageFootprint` existent, et `V10` (`8dbb05e`) les affiche. **Jamais vérifiable avant le prompt 21**, voir sa fiche : rien n'a tourné contre un vrai conteneur CloudKit |
