...
| **La correction en masse depuis l'aperçu (`11f`) n'est pas livrée**, ni l'abandon avec reprise de brouillon (`11g`). `ImportCorrection` et `ImportDraftStore` existent côté cœur : c'est du travail d'écran. L'aperçu affiche donc « à corriger dans le fichier » à côté de chaque cause, au lieu d'une action de masse qui ne corrigerait rien | `V8` second jalon |
| **La correspondance mémorisée n'est pas rejouée.** `ImportMappingRepository` rend un `ImportMapping` du magasin, `ColumnMatcher.analyze` attend un `ColumnMapping` décodé, et **le pont entre les deux n'existe pas**. Conséquence : un fichier déjà importé refait ses déductions au lieu de retrouver les choix de l'utilisateur, et la case « Mémoriser » du bloc `11d` n'est pas rendue. `ColumnMapping` est versionné et relu par `analyze(remembered:)` — tout est là sauf la jointure | `V8` second jalon |
...
| 22 | `V8` — import et export : aperçu ligne à ligne, corrections, progression | `L11a` `L11b` `L12` sont faites et n'ont **aucun** écran. Sept écarts inscrits y pointent | légère — 🔶 **premier jalon `12a8b8a`** : `11d` → `11e` → écriture → `11j`, plus l'export et le modèle vide. **Découpé plutôt que reporté une troisième fois.** Second jalon : `11f` (correction en masse), `11g` (abandon et reprise), et le rejeu de la correspondance mémorisée |
