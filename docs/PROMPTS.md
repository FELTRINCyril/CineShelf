...
| **Aucun test ne couvre l'écran de corbeille au-delà du rendu.** La sonde prouve qu'il dessine et que vide ≠ peuplé ; la **restauration par le bouton** — donc `TrashView.restore` et son `save()` — n'est exercée par rien. `TrashService.restore` l'est côté cœur, mais la couture entre les deux ne l'est pas, exactement comme celle de l'import avant sa sonde de bout en bout | un test d'interface |
