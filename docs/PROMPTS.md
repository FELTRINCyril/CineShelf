...
| **Le repliage de texte est invariant de locale depuis le 2026-08-04 — audit fait, ne pas régresser.** `sortName`, `searchText` et `nameKey` sont persistés **et synchronisés par CloudKit** : les replier avec `locale: .current` faisait écrire des valeurs différentes selon l'appareil. Mesuré : « Interstellar » se replie en `ınterstellar` en turc, « ITALIA » en `ıtalıa` — ce n'est pas exotique, **tout mot à `I` majuscule** diverge. Le site qui mordait vraiment est `Genre.nameKey`, qui sert au dédoublonnage : deux appareils désaccordés créent un doublon **en silence**. Les 12 sites passent par `String.foldedForMatching` (`TextFolding.swift`), seule locale du dépôt, `en_US_POSIX`. **Aucune migration** : `fr_FR` et `en_US_POSIX` replient identiquement, donc les valeurs déjà en base sont inchangées — seul un appareil turc aurait divergé. Deux filets, parce qu'un seul ne suffit pas : 11 tests d'invariance, **et** la règle `no_folding_outside_text_folding` — vérifié, un modèle qui contourne `TextFolding` passe tous les tests sur une machine française | permanent |
