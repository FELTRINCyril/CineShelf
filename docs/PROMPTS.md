...
| **Les genres épinglés n'ont plus aucun point d'accès — QUESTION DE DESIGN OUVERTE, pas un retard.** C'est une fonctionnalité de `docs/03`, et son seul point d'entrée était la barre latérale que `V0` supprime. La barre haute est pleine (six sections + recherche + sync + avatar) et un placement provisoire dans le menu de profil serait exactement ce qu'on ne pourrait plus déloger. **Piste à confirmer contre les planches, pas à décider maintenant** : l'accueil affiche déjà des rails par genre (planche 1 bloc `2a`, planche 2 bloc `3a` — « Mes genres · Drame »), donc les genres épinglés pourraient cesser d'être une fonctionnalité de **navigation** pour devenir la **configuration de l'accueil** — quels rails, dans quel ordre. À trancher en `V5a` (accueil) ou `V5b` (genres) | `V5a` · `V5b` — question ouverte |
...
| 6 | `V0 bis` — **titres** : grille, fiche, éditeur | Remplace le prompt 11. C'est l'écran où l'app se juge | légère — 🔶 **grille faite** `ef4ed7e` · fiche et éditeur non faits |
